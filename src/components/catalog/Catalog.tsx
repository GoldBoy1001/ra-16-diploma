...
export default function Catalog() {
  ...
  const { isLoading, isError } = useProductsQuery("");
  ...
  return (
    <div>
      ...
      {!isError && pathname === "/catalog" && (
        <SearchForm searchCategory={clickedCategory} />
      )}
      {!isLoading && !isError && <Categories />}
      {preload && !isError && <Preload />}
      ...
      {!isError && !preload && more && (
        <Button
          category={clickedCategory}
          onMore={() => [setOnMore(true), handleClick()]}
        />
      )}
    </div>
  );
