...
export default function OrderCart() {
	...
	const items = cart.map(obj => {
		const {title, images, category, ...rest} = obj
		return {...rest, count: 1}
	});
	...
	async function submitHandler(e: React.FormEvent<HTMLFormElement>) {
		...
		const response = await axios.post('http://localhost:7070/api/order', productData)
              ...
              <form onSubmit={submitHandler} className="card-form-body">
                ...
                <div className="form-group">
                  ...
                  <input 
						...
						className="form-control form-control-margin" id="address" placeholder="Адрес доставки"/>
                </div>
                <div className="form-group form-check">
                  <input type="checkbox" className="form-check-input" id="agreement"/>
                  <label className="form-check-label" htmlFor="agreement">Согласен с правилами доставки</label>
                </div>
                <button type="submit" className="btn btn-outline-primary">Оформить</button>
              </form>
            </div>
          </section>
)
